...
CREATE TABLE `t_process` (
  ...
  `file_path` varchar(500) NOT NULL COMMENT '流程zip文件路径',
...
CREATE TABLE `t_sys_log` (
  ...
  `execute_class` varchar(200) DEFAULT NULL COMMENT '执行的类名称',
  `execute_method` varchar(200) DEFAULT NULL COMMENT '执行的方法名称',
  `execute_time` datetime DEFAULT NULL COMMENT '开始执行时间',
  `execute_cost_time` bigint(20) DEFAULT NULL COMMENT '执行耗时，单位为ms',
  `execute_ip` varchar(100) DEFAULT NULL COMMENT '执行ip地址',
  PRIMARY KEY (`id`),
...
CREATE TABLE `t_user` (
  ...
  `salt` varchar(200) DEFAULT NULL COMMENT '加密盐值，随机生成',
  `create_time` datetime DEFAULT CURRENT_TIMESTAMP COMMENT '创建时间',
  `update_time` datetime DEFAULT NULL COMMENT '更新时间',
  `is_active` tinyint(4) DEFAULT '0' COMMENT '是否激活',
  PRIMARY KEY (`id`)
